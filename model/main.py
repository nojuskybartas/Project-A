...
def train_the_model(epochs, batch_size, window_len, lstm_neurons, loss, dropout, optimizer, X_train, y_train, graph=True, summary=True):
    print('\n')
...
def test_the_model(model_folder, max_size, graph=True):
    ...
    #     preds = preds.reshape(len(X_test[:max_size]), WINDOW_LEN)
    print(preds)
    if graph:
